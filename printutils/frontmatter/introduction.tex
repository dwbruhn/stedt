...
\end{tabular}
\caption{TB languages by number of speakers}
\label{tbl:tb_pops}
\end{table}
...
\caption{Schematic Chart of Sino-Tibetan Languages}\footnotemark
\label{fig:STC_tree}
\end{figure}
\footnotetext{Reproduced from \textit{\citetalias{STC}}, p.~6;
\textit{\citetalias{JAM-VSTB}}, p.~3; \textit{\citetalias{JAM-HPTB}}, p.~4.}

A simpler scheme has been used heuristically at STEDT until very recently (see \textit{\citetalias{JAM-HPTB}}:5 and \textit{\citetalias{JAM-TBRS}}:xxix), as shown in  See Fig.~2. It differs from \textit{\citetalias{STC}} in several respects:
...
The PTB vowel system in open syllables is displayed in Figure XYZ (see \textit{\citetalias{JAM-HPTB}}:159–160). Low frequency open-syllable rhymes are in parentheses or square brackets:
