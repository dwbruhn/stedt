...
The Ci may be followed by one of four glised */-w-, -y-, -r-, -l-/, or even by certain sequences of two of them.

[get PTB vowel system open sylls from .... where???]
...
FN Paper is not a problem electronically, but in printed publications (e.g. TBRS) the form only appears once, but there are  multiple attributions, separated by commas. An adjustment know as the “Mariama fix” [spelling?], whereby absolutely identical recoreds are collapsed into one, has not yet been implemented for the database as a whole.
...
\mbox{written} with the ‘allofam symbol’ \STEDTU{x74} between them, e.g.: \textbf{*glim} \STEDTU{x74} \textbf{*glip}
BROOD~/ INCUBATE; \textbf{*s-rix55} \STEDTU{x74} \textbf{*s-r(y)ax55} LIVE / ALIVE / GREEN / RAW / GIVE BIRTH.
Sometimes, however, I use an alternative notation with parentheses, e.g.: \textbf{*(t)si}
COPULATE/LOVE; this is equivalent to \textbf{*si} \STEDTU{x74} \textbf{*tsi}.  Slashes may also be used,
e.g.\ \textbf{*p/buk} \STEDTU{x74} \textbf{*p/bik} BORN/GIVE BIRTH; this is equivalent to
\textbf{*puk} \STEDTU{x74} \textbf{*buk} \STEDTU{x74} \textbf{*pik} \STEDTU{x74} \textbf{*bik}.
...
This means the same as \textbf{*tux55} \STEDTU{x74} \textbf{*dux55.}
\end{quote}
...
dental plus \textbf{-y-},
rather than writing them with \textit{hax50eks} or grave accents, e.g.:
...
[find and insert "pseudo-micritizing" text..]
