	...
	$A(tbody.rows).each(function (row, j) {
		...
		if (lastord != ord) {
			...
			if (stedtuserprivs & 1) {
				...
				+ row.cells[grp_index].innerHTML.replace(/"/g,'&quot;') + '?\')"><input name="tag" value="' + table_tag
				+ '" type="hidden"><input name="uid" value="' + uid2 + '" type="hidden"><input name="ord" value="'
				+ ord + '" type="hidden"><input type="submit" value="Accept ' + row.cells[grp_index].innerHTML.replace(/"/g,'&quot;') + ' only"></form>';
			}
			lastord = ord;
		}
	});
